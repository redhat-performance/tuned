...
class ThreadNoLongerExists(Exception):
	"""A thread disappeared while processing it"""
	pass

# another custom exception to signal non-changeability of affinities
class AffinityNotChangeable(Exception):
	"""Affinity of a thread cannot be changed"""
	pass

class KthreadInfo(object):
	"""Kernel thread information for internal bookkeeping"""
	...
	def __str__(self):
		"""String representation of a kthread, for logging purposes"""
		return "%d:%s" % (self.pid, self.comm)

class SchedOpts(object):
	"""Scheduling options for a kthread, as parsed from the group.* options"""
...
class GroupCmd(object):
	"""group.* definition from the instance options"""
...
class KthreadPlugin(base.Plugin):
		...
		if have_perf:
			try:
				...
				try:
					...
					pass
				have_perf = False
				log.warning("perf initialization failed: %s. " \
					"Tuning will be applied to all currently running kthreads, but future kthreads will not be tuned." % e)
		else:
	...
	def cleanup(self):
		"""Cleanup the plugin, unapplying tuning and stopping the perf monitor thread"""
	...
	def _init_devices(self):
		"""Initialize devices for the plugin"""
	...
	def _get_config_options(cls):
		"""Get plugin options. We have none, all options are instance-level"""
		return {
	...
	def _plugin_add_kthread(self, pid):
		...
		# Prefer active instances so the kthread gets tuned immediately
		for instance in self._instances.values():
			if instance._tuning_active and self._get_matching_kthreads(instance, [pid]):
				self._instance_add_kthread(instance, pid)
				return
		for instance in self._instances.values():
			if not instance._tuning_active and self._get_matching_kthreads(instance, [pid]):
				self._instance_add_kthread(instance, pid)
				return
		...
		for k, v in instance.options.items():
			...
			if opt_schedopts in ["", "*"]:
				...
				try:
					...
					continue
				prio_min = self._scheduler_utils.get_priority_min(policy)
				prio_max = self._scheduler_utils.get_priority_max(policy)
				policy_name = self._scheduler_utils.sched_num_to_const(policy)
				if priority < prio_min or priority > prio_max:
					log.error("Could not parse scheduling priority for '%s': %s requires priority in range %d - %d, got %d" % (k, policy_name, prio_min, prio_max, priority))
					continue
			else:
		...
		with self._lock:
			if instance._tuning_active:
				try:
					...
					log.error("Failed to tune kthread %d: %s" % (pid, e))
					self._kthread_pids_unassigned.add(pid)
					return
	...
	def _instance_transfer_kthread(self, instance_from, instance_to, pid):
		...
		with self._lock:
			was_processed = pid in instance_from._kthreads_processed
			if was_processed:
			...
			if instance_to._tuning_active:
				try:
					...
					log.error("Failed to tune kthread %d during transfer: %s" % (pid, e))
					# Roll back: return PID to source instance
					if was_processed:
						instance_from._kthreads_processed.add(pid)
					else:
						instance_from._kthreads_assigned.add(pid)
			else:
		...
		with self._lock:
			for other_instance in self._instances.values():
				if (other_instance == instance or instance.priority > other_instance.priority or not hasattr(other_instance, "_kthreads_assigned")):
					continue
				# Don't steal from active instances while we are inactive;
				# defer until our own activation in _instance_apply_static
				if not instance._tuning_active and other_instance._tuning_active:
					continue
	...
	def _instance_release_kthreads(self, instance):
		...
		with self._lock:
			...
			# the remaining ones go back to unassigned (only if still alive)
			for pid in free_kthreads:
				self._instance_remove_kthread(instance, pid)
				if pid in self._kthreads:
					self._kthread_pids_unassigned.add(pid)

	def _instance_apply_static(self, instance):
		"""Apply instance static tuning"""
		with self._lock:
			...
			for pid in list(instance._kthreads_assigned):
				...
				try:
					...
					continue
				instance._kthreads_processed.add(pid)
		# Now active, acquire kthreads from lower-priority instances
		# that were deferred while we were inactive
		self._instance_acquire_kthreads(instance)

	def _instance_verify_static(self, instance, ignore_missing, devices):
		"""Verify instance static tuning"""
		result = True
		with self._lock:
	...
	def _instance_unapply_static(self, instance, rollback):
		"""Unapply instance static tuning, optionally rolling back to original settings"""
		with self._lock:
			instance._tuning_active = False
			for pid in list(instance._kthreads_processed):
				try:
	...
	def _kthread_internal_remove(self, pid):
		...
		try:
			with self._lock:
				...
				self._kthread_pids_unassigned.discard(pid)
	...
	def _perf_monitor_thread(self):
		...
		class NewThread(object):
			"""Helper class to keep track of new threads and their delay"""
		...
		while not self._terminate.is_set():
			try:
				...
				log.error("perf monitor thread encountered an error: %s" % e)
				# back off to avoid a tight error-spin loop on a persistent failure
	...
	def _get_schedopts(self, pid):
		"""Get scheduling policy and priority of a thread"""
		try:
	...
	def _format_schedopts(self, policy, priority):
		"""Format scheduling policy and priority as string"""
		...
		if opts.policy is not None or opts.priority is not None:
			if opts.policy != current_policy or opts.priority != current_priority:
				try:
					...
					if affinity_changed:
						try:
							self._set_affinity(kthread.pid, current_affinity)
						except Exception:
							pass
					raise

		kthread.sched_orig = SchedOpts(policy=current_policy, priority=current_priority, affinity=current_affinity)
		if affinity_changeable is not None:
			kthread.affinity_changeable = affinity_changeable
		kthread.tuned_affinity = affinity_changed
		kthread.tuned_sched = sched_changed

	def _change_kthread_tuning(self, kthread, opts):
		"""Change tuning of a kthread"""
		prev_affinity = self._get_affinity(kthread.pid)
		prev_tuned_affinity = kthread.tuned_affinity
		affinity_changed = False
		...
		if opts.policy is None and opts.priority is None and kthread.tuned_sched:
			...
			try:
				...
				if affinity_changed:
					try:
						self._set_affinity(kthread.pid, prev_affinity)
						kthread.tuned_affinity = prev_tuned_affinity
					except Exception:
						pass
				raise

	def _restore_kthread_tuning(self, kthread):
		"""restore tuning of a kthread to its original settings"""
	...
	def _verify_kthread_tuning(self, kthread, opts):
		"""verify that the tuning of a kthread matches the desired settings"""
